...
const Signin = () => {
  ...
  // const redirectUri = config.GmailAuthorization.URL.AuthURL;

  useEffect(() => {
    const gmailStatus = new URLSearchParams(location.search)
      .get("gmail")
      ?.trim()
      .toLowerCase();

    if (gmailStatus === "success") {
      navigate(all_routes.datatable, { replace: true });
    } else if (gmailStatus === "failed") {
      setErrorMessage("Gmail authorization failed. Please try again.");
    }
