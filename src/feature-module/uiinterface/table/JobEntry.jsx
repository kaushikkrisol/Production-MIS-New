...
const pasteableLineColumns = [
  ...
  // Estimate screen paste order

  { key: "billLoc", label: "Region", aliases: ["region", "bill loc", "billing location", "billing loc"] },

  { key: "store", label: "Salon Name", aliases: ["salon name", "salon", "store", "store name", "salon/store name"] },

  { key: "articleCode", label: "Article Code", aliases: ["article code", "articlecode", "visual code", "visualcode"] },
  ...
  { key: "hsn", label: "HSN Code", aliases: ["hsn", "hsn code", "hsncode"] },
...
const normalizePastedLineValue = (field, value) => {
  ...
  if (["qty", "rate", "sqft"].includes(field)) {
...
const applyPastedLinePatch = (line, patch) => {
  ...
  // Article Code and Visual Code are the same value for estimate/job rows.

  // Keep both in sync so PDF, mail and existing job-entry save logic read the correct code.

  if (Object.prototype.hasOwnProperty.call(syncedPatch, "articleCode") && !syncedPatch.visualCode) {

    syncedPatch.visualCode = syncedPatch.articleCode;

  }

  if (Object.prototype.hasOwnProperty.call(syncedPatch, "visualCode") && !syncedPatch.articleCode) {

    syncedPatch.articleCode = syncedPatch.visualCode;
...
const JobEntry = () => {
  ...
  const getRowsMissingRequiredLocations = () =>
    lines
      ...
      .map(({ line, rowNo }) => {
        const missingFields = [];
        if (!String(line.prodLoc || "").trim()) missingFields.push("Production Location");
        if (!String(line.billLoc || "").trim()) missingFields.push("Billing Location");
  ...
  const showRequiredLocationWarning = (missingRows) => {
    const preview = missingRows
      .slice(0, 5)
      .map(({ rowNo, missingFields }) => `Row ${rowNo}: ${missingFields.join(", ")}`)
      .join("; ");
    const extraCount = missingRows.length - 5;
    const message = `Please select mandatory location field(s): ${preview}${extraCount > 0 ? ` and ${extraCount} more row(s)` : ""}.`;
  ...
  const saveExistingJobLines = async () => {
    ...
    const missingRequiredLocations = getRowsMissingRequiredLocations();



    if (missingRequiredLocations.length) {

      showRequiredLocationWarning(missingRequiredLocations);
  ...
  const fetchEstimates = useCallback(async () => {
    ...
    try {
      ...
      const estimateUrl = getEstimateApiUrl();

      if (!estimateUrl) {

        throw new Error("GetEstimate URL is missing in config.JobSummary.URL.GetEstimate");

      }

      const response = await axios.get(estimateUrl, { timeout: 10000 });

      setEstimateRows(getEstimateRowsFromResponse(response.data));

    } catch (error) {

      console.error("Error fetching estimates", error);

      toast.error(error?.response?.data?.message || error?.message || "Failed to load estimates");
      ...
      <main className="job-entry-shell">
          ...
          <div className="job-entry-actions">
            ...
            {activeJobEntryTab === "existing" ? (
             ...
             <button type="button" title="Download PDF" onClick={downloadEstimateAttachment}>
              ...
              <button className="job-secondary-btn" type="button" onClick={fetchEstimates} disabled={isLoadingEstimates}>

                <RotateCcw size={16} />

                <span>{isLoadingEstimates ? "Loading" : "Refresh Estimates"}</span>
        ...
        {activeJobEntryTab === "estimateList" ? (
          ...
          <section className="job-lines-wrap">
            ...
            <div className="job-lines-toolbar">

              <h4 style={{ margin: 0 }}>Estimate List</h4>

              <button type="button" onClick={fetchEstimates} disabled={isLoadingEstimates}>

                <RotateCcw size={16} />

                <span>{isLoadingEstimates ? "Loading" : "Refresh"}</span>
                ...
                <tbody>
                  ...
                  {isLoadingEstimates ? (
                    ...
                    estimateRows.map((item, index) => (
                      ...
                      <tr key={item.id || item._id || item.estimateNo || item.EstimateNo || index}>

                        <td>{item.estimateNo || item.EstimateNo || "-"}</td>

                        <td>{item.jobNo || item.JobNo || "-"}</td>

                        <td>{item.client || item.Client || item.customerName || item.CustomerName || "-"}</td>

                        <td>{item.projectName || item.ProjectName || "-"}</td>

                        <td>{item.mailTo || item.MailTo || "-"}</td>

                        <td>{item.totalSqFt || item.TotalSqFt || "-"}</td>
                        ...
                        <td>{item.createdBy || item.CreatedBy || item.enteredby || item.Enteredby || "-"}</td>
          ...
          <>
            ...
            {entryScreenMode === "estimate" ? (
              ...
              <section
                ...
                style={{ minHeight: "auto", height: "auto", marginBottom: "12px" }}

              >

                <div

                  className="job-lines-scroll estimate-lines-scroll"
                          ...
                          <tr key={line.id}>
                            ...
                            <td className="select-col">
                              ...
                              <input
                                ...
                                aria-label={`Select estimate row ${index + 1}`}
                            ...
                            <td>
                              ...
                              <Select
                                ...
                                value={getSelectedOption(locationSelectOptions, line.billLoc || line.prodLoc)}
                            ...
                            <td>
                              ...
                              <input
                                ...
                                value={line.articleCode || line.visualCode || ""}

                                onChange={(event) => {

                                  updateLine(line.id, "articleCode", event.target.value);

                                  updateLine(line.id, "visualCode", event.target.value);

                                }}

                                {...getPasteCellProps(line.id, "articleCode")}
